...
total = my_data.apply(lambda x: x['Prix'] * x['Quantite'] * (1 - x['Reduction'] * 0.01), axis=1).sum()
print("le total de la recette vendue= ",total)

#2
my_data = pd.read_excel("../DDB/Recette.xlsx")
total_by_day = my_data.groupby('Jour').apply(lambda x: (x['Prix'] * x['Quantite'] * (1 - x['Reduction'] * 0.01)).sum()).reset_index(name='Total')
print(total_by_day)
